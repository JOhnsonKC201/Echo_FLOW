...
1. Pull the latest Echo Flow and run `scripts\setup.bat` (or `pip install -r requirements.txt` in your venv), which picks up the new `flask` and `zeroconf` deps.
...
|---------------------|------|-----------------------------------|--------------------------------------------------------|
| `GET  /v1/health`   | no   | none                              | `{ok, phase, providers, model_loaded}`                 |
