...
- VRAM footprint: `large-v3-turbo` float16 ≈ 1.5 to 2 GB VRAM (estimate; not measured). `base` int8 on CPU uses RAM, not VRAM.
...
- `condition_on_previous_text=False` (`transcribe.py:56`), which avoids cross-utterance contamination.
...
- Configured model: `qwen3.5:latest` (`config.yaml:70`). Default fallback in code is `qwen2.5:7b-instruct` (`cleanup.py:393`). Estimated VRAM at Q4/Q5 ≈ 5 to 7 GB.
- Quantization: not specified in config, so it relies on whichever tag Ollama has pulled.
...
Headless? Almost: the tray icon is always created (`main.py:628-641`). No console required, but a tray icon is mandatory.
...
- Embedding model: **CPU**, since sentence-transformers `all-MiniLM-L6-v2` is small enough that the code doesn't explicitly move it to CUDA.
- Silero VAD: runs on CPU via tiny torch tensors (`audio.py:104-108`).

### VRAM budget on RTX 5060 (8 GB)
Estimate (not measured):
- faster-whisper `large-v3-turbo` fp16: ~1.5 to 2 GB
- Ollama `qwen3.5:latest` (described as "~6.5 GB" at `config.yaml:70`): ~6.5 GB

**Total: ~8 to 8.5 GB.** This is over budget for an 8 GB card. Either Ollama will swap layers to CPU (latency hit), or the user must downgrade one of the two. The config comment claims qwen3.5 is 9.7B at ~6.5 GB but doesn't address coexistence with Whisper on the same card.
...
CHANGELOG claims "81 tests"; actual file count is 9 files (test functions likely ~60 to 80, not enumerated).

Coverage estimate: cleanup providers (`_via_groq/_via_ollama/_via_anthropic/_via_openai`) are not tested (would require HTTP mocks). `transcribe.py` and `audio.py` are not tested (require hardware/model). Estimated line coverage ~40 to 50%.
...
- ASR (large-v3-turbo fp16 on RTX 5060, 5 s clip with beam=5, language pinned): **~400 to 700 ms** estimate
- Cleanup via Ollama (qwen3.5 ~9.7B, 5 to 20 token output): **~600 to 1500 ms** estimate (assumes warm model in VRAM; cold-start adds seconds)
- Inject (clipboard copy + Ctrl+V): ~20 ms (`inject.py:48` has 8 ms sleep + pyautogui latency)
- **Total: ~1.0 to 2.2 s** before text appears.

**Phase 1 / Bootstrap (Groq Whisper + Groq cleanup):**
- ASR upload + Groq turbo: **~300 to 500 ms** (Groq is fast; depends on internet)
- Cleanup Groq llama-3.1-8b-instant: **~80 to 200 ms**
- **Total: ~400 to 700 ms.** This is faster than fully-local, which is precisely why phasing defaults to it for the first 50 uses.

**Phase 4 / Self-Sufficient (local Whisper + `learned` provider, no LLM):**
- ASR: same ~400 to 700 ms
- Cleanup (token substitutions + `_polish_text`): **<10 ms**
- **Total: ~450 to 750 ms.** Fastest local option, but requires 2000+ dictations of history and ≥75 avg quality (`phase.py:124-133`).
...
- `GROQ_API_KEY` is set AND phasing is enabled AND history <50 → auto-routes to Groq, will fail with network error (which `HybridTranscriber.transcribe` does not handle if backend is pure `groq`, see `transcribe_cloud.py:53-59`, which will raise on `r.raise_for_status()` and crash the dictation thread).
