...
| DASH-2 | `action_apps` editor (mapping-aware writer OR SQLite-backed store, since config_writer is scalar-only) | Dashboard | M | med |
...
Write to `Path("data")/"drafts"` (relative, since frozen builds chdir to USER_ROOT,
