...
must return the raw text unchanged with polish_skipped=False, and notify.notify
must be called exactly once at "error" severity.
"""
...
def test_via_ollama_error_returns_raw_and_notifies(monkeypatch, fake_post):
    ...
    assert out == RAW, f"raw must be returned on error; got {out!r}"
